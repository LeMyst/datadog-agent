...
func AdditionalEndpointsWithNames(c pkgconfigmodel.Reader, setting string, value interface{}) map[string][]map[string]string {
	...
	named := MakeNamedEndpoints(resolved, originalStringMapStringSlice(c, setting), setting)
...
// originalStringMapStringSlice returns the user-configured value of setting
// with the secret layer excluded, so callers can see ENC[handle] entries as
// the user wrote them instead of the resolved key material. Uses the config's
// own non-secret merged view so source priority is handled centrally.
func originalStringMapStringSlice(c pkgconfigmodel.Reader, setting string) map[string][]string {
...
func GetMultipleEndpoints(c pkgconfigmodel.Reader) (EndpointDescriptorSet, error) {
	...
	additionalEndpoints := MakeNamedEndpoints(
		...
		originalStringMapStringSlice(c, "additional_endpoints"),
