...
func TestRuntimeSecurityLoadedPolicies(t *testing.T) {
	...
	t.Run("system-probe unreachable", func(t *testing.T) {
		withFakeRuntimeSecurityClient(t, nil, fmt.Errorf("dial unix /var/run/system-probe.sock: connection refused"))
	...
	t.Run("gRPC call fails", func(t *testing.T) {
		fake := &fakeRuntimeSecurityCmdClient{getErr: fmt.Errorf("rpc error: code = Unavailable")}
		withFakeRuntimeSecurityClient(t, fake, nil)
...
func TestProvideSecurityAgent_FeatureGating(t *testing.T) {
	...
	t.Run("both features disabled", func(t *testing.T) {
		...
		withFakeRuntimeSecurityClient(t, &fakeRuntimeSecurityCmdClient{}, fmt.Errorf("client must not be created"))
